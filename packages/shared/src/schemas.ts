...
export const createOrderSchema = z.object({
  ...
  /** โหมดเปิดบิลภัตตาคาร — ไม่ส่ง payment / member จนกว่าจะปิดเซสชัน */
  dining_session_id: z.string().uuid().optional(),
  ...
  if (d.dining_session_id) {
    if (d.payment_method != null) {
      ctx.addIssue({ code: 'custom', path: ['payment_method'], message: 'Open-tab rounds must not include payment_method' });
    }
    if (d.customer_id) {
      ctx.addIssue({ code: 'custom', path: ['customer_id'], message: 'Attach member when closing the bill' });
    }
    if ((d.points_redeemed ?? 0) > 0) {
      ctx.addIssue({ code: 'custom', path: ['points_redeemed'], message: 'Use points when closing the bill' });
    }
  }
});

export const closeDiningSessionSchema = z.object({
