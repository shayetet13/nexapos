...
import { ShopModesTab }    from '@/components/dev/ShopModesTab';
import { ReceiptTuningTab } from '@/components/dev/ReceiptTuningTab';
...
<p class="footer">สร้างโดย NexaPos Dev Console · ${new Date().toLocaleString('th-TH')}</p>
  ...
  const tabs: { id: TabId; icon: string; label: string; desc: string }[] = [
    ...
    { id: 'shop_modes', icon: '🏪',  label: 'ระบบร้าน',     desc: 'เปิด/ปิดระบบร้านที่รองรับ' },
    { id: 'receipt',    icon: '🧾',  label: 'ใบเสร็จ',       desc: 'ปรับขนาด/ระยะ + พิมพ์ทดสอบ' },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className={`${activeTab === 'overview' ? 'max-w-6xl' : 'max-w-4xl'} mx-auto space-y-5`}>
            ...
            {activeTab === 'shop_modes' && <ShopModesTab />}
            {activeTab === 'receipt'    && <ReceiptTuningTab />}
