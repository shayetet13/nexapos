  ...
  return (
    <div className="refund-page">
      ...
      <div className="refund-steps">
        {(['search', 'confirm', 'otp', 'done'] as Step[]).map((s, i) => {
          const labels = ['ค้นหาออเดอร์', 'เลือกประเภท', 'ใส่รหัส', 'สำเร็จ'];
          const active  = s === step;
          const done    = (['search', 'confirm', 'otp', 'done'] as Step[]).indexOf(s) <
                          (['search', 'confirm', 'otp', 'done'] as Step[]).indexOf(step);
          return (
            <div key={s} className={`refund-step ${active ? 'refund-step--active' : ''} ${done ? 'refund-step--done' : ''}`}>
