    ...
    async function init() {
      ...
      if (assignRes.ok) {
        ...
        if (d.role === 'cashier' || d.role === 'viewer') {
          const p = new URLSearchParams({ shopId: d.shopId, shopName: d.shopName ?? '', branchId: d.branchId ?? '', branchName: d.branchName ?? '' });
          window.location.href = d.branchId ? `/pos?${p}` : '/select-shop';
          return;
        }
      }
