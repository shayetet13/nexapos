...
export default function RegisterPage() {
  ...
  const [shopMode,       setShopMode]       = useState<string>('retail');
  const [province,       setProvince]       = useState('');
  const [district,       setDistrict]       = useState('');
  const [availableModes, setAvailableModes] = useState<ShopModeOption[] | null>(null);
  ...
  // ── Load available shop modes when entering step 3 ──────
  useEffect(() => {
    if (step !== 3 || availableModes !== null) return;
    void (async () => {
      try {
        const res  = await fetch(`${PROXY_BASE}/shop-modes`);
        const json = await res.json() as { success: boolean; data: ShopModeOption[] };
        if (json.success && Array.isArray(json.data)) {
          setAvailableModes(json.data);
          if (json.data.length > 0 && !json.data.find((m) => m.key === shopMode)) {
            setShopMode(json.data[0].key);
          }
        }
      } catch {
        // ถ้า fetch ล้มเหลว ให้แสดงทุก mode ตาม fallback
      }
    })();
  ...
  function goNext() {
    ...
    if (step === 3) {
      if (availableModes !== null && availableModes.length === 0) {
        setError('ขณะนี้ระบบปิดรับการสมัครชั่วคราว');
        return;
      }
      ...
      <div className="reg-card">
        ...
        {step === 3 && (
          <div className="reg-form">
            ...
            <p className="reg-form__desc">ประเภทร้าน (เลือกครั้งเดียวตอนเปิด)</p>
            {availableModes === null ? (
              <div className="reg-mode-row">
                <div className="reg-mode-card reg-mode-card--skeleton" aria-hidden="true" />
                <div className="reg-mode-card reg-mode-card--skeleton" aria-hidden="true" />
              </div>
            ) : availableModes.length === 0 ? (
              <div className="reg-email-note">
                🔧 ขณะนี้ระบบปิดรับการสมัครชั่วคราว กรุณากลับมาใหม่ในภายหลัง
              </div>
            ) : (
              <div className="reg-mode-row" role="group" aria-label="ประเภทร้าน">
                {availableModes.map((mode) => (
                  <label
                    key={mode.key}
                    className={`reg-mode-card${shopMode === mode.key ? ' reg-mode-card--active' : ''}`}
                  >
                    <input
                      type="radio"
                      name="shopMode"
                      className="reg-mode-card__input"
                      checked={shopMode === mode.key}
                      onChange={() => setShopMode(mode.key)}
                    />
                    <span className="reg-mode-card__title">{mode.label}</span>
                    <span className="reg-mode-card__hint">{mode.hint}</span>
                  </label>
                ))}
              </div>
            )}
        ...
        {step === 5 && (
          <div className="reg-success">
            ...
            <div className="reg-success__actions">
              ...
              {shopMode === 'full_service_restaurant' ? (
                <Link href="/login" className="reg-success__link">เข้าสู่ระบบเพื่อเปิดภัตตาคาร</Link>
              ) : (
