...
export default function RegisterPage() {
  ...
  function goNext() {
    ...
    if (step === 4) { void handleSubmit(); return; }
  ...
  return (
    <main className="reg-page">
      <div className="reg-card">
        ...
        {step === 4 && (
          <div className="reg-form">
            <h2 className="reg-form__title">🏢 สาขา <span className="reg-form__optional">(ไม่บังคับ)</span></h2>
            <p className="reg-form__desc">ถ้ามีสาขาให้ใส่ชื่อสาขา ถ้าไม่มีสามารถข้ามได้</p>
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && goNext()}
            />
            <p className="reg-skip-hint">กดปุ่ม <strong>ข้ามขั้นตอนนี้</strong> เพื่อสร้างร้านโดยไม่มีสาขา (เพิ่มสาขาได้ภายหลัง)</p>
          </div>
        )}
        ...
        {step !== 1 && step !== 5 && (
          <div className="reg-nav">
            ...
            <button
              type="button"
              className={`btn-primary reg-nav__next${step === 4 && !branchName.trim() ? ' reg-nav__next--skip' : ''}`}
              onClick={goNext}
              disabled={loading}
            >
              {loading
                ...
                : step === 4
                  ? (branchName.trim() ? 'สร้างร้านค้า ✓' : 'ข้ามขั้นตอนนี้ →')
                  : 'ถัดไป →'}
            </button>
          </div>
        )}
