...
/* ── Order-sequence helpers (localStorage per shop, resets at 23:59 daily) ── */
...
function loadSeq(shopId: string): number {
  try {
    ...
    if (storedDate !== todayStr()) {
      localStorage.setItem(`pos_seq_date_${shopId}`, todayStr());
...
function POSContent() {
  ...
  useEffect(() => {
    if (!shopId) return;
    function checkAndReset() { setOrderNumber(loadSeq(shopId!)); }
      ...
      <header className="pos-topnav">
        ...
        <div className="pos-topnav__right">
          ...
          <div className="pos-avatar-wrap">
            ...
            {userMenuOpen && (
              <>
                <div className="pos-user-overlay" onClick={() => setUserMenuOpen(false)} />
                <div className="pos-user-menu">
                  {(userRole === 'owner' || userRole === 'manager') && (
                    <button className="pos-user-menu__item" onClick={() => { setUserMenuOpen(false); setAdminOpen(true); }}>
                      🔐 {t.admin.menuLabel}
                    </button>
                  )}
                  <Link href={`/refund?shopId=${shopId}&branchId=${branchId}`} className="pos-user-menu__item" onClick={() => setUserMenuOpen(false)}>
                    💰 คืนเงิน
                  </Link>
                  <Link href={backHref} className="pos-user-menu__item" onClick={() => setUserMenuOpen(false)}>{t.back}</Link>
                  <button
              ...
              {products.map(p => {
                ...
                return (
                  <button
                    key={p.id}
                    onClick={() => addToCart(p)}
                    className={['pos-card', inCart ? 'pos-card--active' : '', isOutOfStock ? 'pos-card--oos' : '', isLowStock ? 'pos-card--low-stock' : ''].filter(Boolean).join(' ')}
                  >
                    {isOutOfStock && <div className="pos-card__oos-overlay" aria-hidden="true"><span className="pos-card__oos-label">หมด</span></div>}
                    {isLowStock && <span className="pos-card__low-stock-badge" aria-label={`เหลือ ${trackedQty}`}>⚠ {trackedQty}</span>}
                    {inCart && !isOutOfStock && <span className="pos-card__badge">{inCart.quantity}</span>}
                    <div className="pos-card__icon-wrap">
                      {p.image_url
                        // eslint-disable-next-line @next/next/no-img-element
                        ? <img src={p.image_url} alt={p.name} className="pos-card__img" loading="lazy" />
                        : <div className="pos-card__img pos-card__img--empty">🍵</div>
                      }
                    </div>
                    <div className="pos-card__info">
                      <span className="pos-card__name">{p.name}</span>
                      <span className="pos-card__price">฿{Number(p.price).toLocaleString('th-TH', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                    </div>
                  </button>
                );
              })}
