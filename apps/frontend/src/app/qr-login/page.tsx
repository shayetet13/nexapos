  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 4. Redirect — ภัตตาคาร → /dining, ร้านค้า → /pos
        setStep('success');
        setMessage('เข้าสู่ระบบสำเร็จ! กำลังเข้าระบบ...');
        setTimeout(async () => {
          let target = `/select-branch?shopId=${shop_id}`;
          if (resolvedBranchId) {
            let mode: ShopMode = 'retail';
            try {
              const cr = await fetchWithAuth(`${API_URL}/api/v1/shops/${shop_id}/pos-config`);
              if (cr.ok) {
                const cj = await cr.json();
                mode = (cj.data?.shop_mode as ShopMode) ?? 'retail';
              }
            } catch {
              mode = 'retail';
            }
