...
export default function ReceiptPage() {
  ...
  // Auto-print when opened with ?autoprint=1 — fires after the receipt
  // DOM is painted AND web fonts have loaded (otherwise window.print()
  // captures the skeleton, or renders Thai text with the fallback font).
  useEffect(() => {
    if (!receipt) return;
    if (typeof window === 'undefined') return;
    if (new URLSearchParams(window.location.search).get('autoprint') !== '1') return;
    let cancelled = false;
    const doPrint = () => { if (!cancelled) window.print(); };
