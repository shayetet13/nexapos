...
function AdminPageInner() {
  ...
  async function deleteProduct(productId: string) {
    ...
    const ok = await confirm({
      title: 'ลบสินค้า',
      description: <><strong>{product?.name ?? 'สินค้านี้'}</strong> จะถูกลบออกจากระบบถาวร ไม่สามารถย้อนกลับได้</>,
      variant: 'danger',
      icon: '🗑',
    });
    if (!ok) return;
  ...
  async function createStaff() {
    if (!shopId || !staffNickname.trim() || !/^\d{4,13}$/.test(staffPin)) return;
  ...
  async function saveEditStaffPin() {
    if (!editStaffModal || !shopId || !/^\d{4,13}$/.test(editStaffPin)) return;
    dispatch({ type: 'PATCH_USERS', payload: { editStaffSaving: true, editStaffError: null } });
  ...
  return (
    <main className="page-admin">
      ...
      {/* Support email — fixed bottom-right */}
      <a
