...
export function CartPanel({
  ...
  onChangeQty, onNoteChange, onClear,   onCheckout, onClose,
  onCustomerDisplay,   onRegisterDisplay, shopId, branchId,
  getMaxQty, primaryButtonLabel, onCloseBill,
  uiVariant = 'retail',
}: {
  cart: CartItem[];
  total: number;
  ...
  /** โหมดภัตตาคาร: ป้ายปุ่มหลัก + ปุ่มปิดบิล */
  primaryButtonLabel?: string;
  onCloseBill?: () => void;
  /** ภัตตาคาร: ข้อความและไม่ล็อกจำนวนตามสต็อก */
  uiVariant?: 'retail' | 'dining';
}) {
  const isDining = uiVariant === 'dining';
  const td = t.dining;
  ...
  return (
    <div className="cart-wrap">
      ...
      <div className="cart-head">
        <h2 className="cart-title">{isDining ? td.currentRound : t.currentOrder}</h2>
        <div className="cart-head__actions">
          {cart.length > 0 && (
            <button onClick={onClear} className="cart-clear-btn">{t.clearAll}</button>
          )}
          <span className="cart-order-num">{isDining ? `${td.roundBadge} ` : ''}#{String(orderNumber).padStart(4, '0')}</span>
          {onClose && (
      ...
      <div className="cart-body">
        {cart.length === 0 ? (
          <div className="cart-empty">
            ...
            <p className="cart-empty__hint">{isDining ? td.noItemsHint : t.noItemsHint}</p>
          </div>
        ) : (
          <ul className="cart-list">
            {cart.map(item => (
              <li key={item.product.id} className="cart-item">
                ...
                <div className="cart-item__row">
                  ...
                  <div className="cart-qty">
                    ...
                    <button
                      ...
                      disabled={!isDining && getMaxQty ? item.quantity >= getMaxQty(item.product.id) : false}
                      title={
                        !isDining && getMaxQty && item.quantity >= getMaxQty(item.product.id)
                          ? 'ไม่เกินสต็อก'
                          : undefined
                      }
                    >+</button>
                ...
                <input
                  type="text"
                  className="cart-item__note"
                  placeholder="โน้ต เช่น หวานน้อย, ไม่ใส่น้ำแข็ง…"
                  maxLength={120}
                  value={item.note ?? ''}
      ...
      <div className="cart-foot">
        ...
        {onCloseBill && (
          <button
            type="button"
            onClick={onCloseBill}
            className="btn-checkout btn-checkout--secondary"
