...
export function OrderDetailModal({
  ...
  function handlePrint() {
    // เปิด receipt page popup แทน window.print() ตรงๆ
    // — window.print() พิมพ์ทั้งหน้า POS ซึ่งไม่ใช่สิ่งที่ต้องการ
    const token = detail?.receipt_token;
    if (token) {
      const w = window.open(`/receipt/${token}`, '_blank', 'width=420,height=700');
      if (w) {
