...
import { loadReceiptLayout, type ReceiptData } from '@/lib/receipt-layout';
import { buildReceiptBytes } from '@/lib/receipt-render';
const t = th.pos;
...
export function POSContent({ experience = 'retail' }: { experience?: PosExperience }) {
  ...
  const [shopTaxId, setShopTaxId]               = useState<string | null>(null);
  const [shopOpeningHours, setShopOpeningHours] = useState<string | null>(null);
  const [shopWorkingDays, setShopWorkingDays]   = useState<string | null>(null);
  const [shopGoogleReviewUrl, setShopGoogleReviewUrl] = useState<string | null>(null);
  ...
  // Thai code page hardcoded: CP874/Windows-874 (ESC t 20) — works on Epson & most Chinese ESC/POS printers.
  ...
  useEffect(() => {
    ...
    fetchWithAuth(`${API_URL}/api/v1/shops/${shopId}/pos-config`).then(async (res) => {
      ...
      // Receipt print shop details
      setShopPhone(json.data?.phone ?? null);
      setShopAddress(json.data?.address ?? null);
      setShopTaxId(json.data?.tax_id ?? null);
      setShopOpeningHours(json.data?.opening_hours ?? null);
      setShopWorkingDays(json.data?.working_days ?? null);
      setShopGoogleReviewUrl(json.data?.google_review_url ?? null);
      // Decode staff nickname from JWT (user_metadata or app_metadata)
      try {
        const { getAuthToken } = await import('@/lib/supabase');
        const token = await getAuthToken();
        if (token) {
          const payload = JSON.parse(atob(token.split('.')[1]!.replace(/-/g, '+').replace(/_/g, '/')));
          const nick = payload?.user_metadata?.nickname ?? payload?.app_metadata?.nickname ?? null;
          setStaffNickname(nick ?? (payload?.email as string | undefined)?.split('@')[0] ?? null);
        }
      } catch { /* ignore */ }
      ...
      try {
        ...
        // Thai code page is hardcoded to 20 (CP874) — no longer configurable per-shop.
      } catch { /* ignore */ }
    });
  }, [shopId]);
  ...
  /** Map a POS order + current shop/staff state → ReceiptData for the renderer. */
  function toReceiptData(order: {
    total: number; cart: CartItem[]; orderNumber: number;
    shopName: string; vatEnabled: boolean; discount: number;
    branchName?: string; paymentMethod?: string;
    subtotal?: number; vatAmount?: number;
    receivedAmount?: number; change?: number;
    refCode?: string; receiptToken?: string;
  }): ReceiptData {
    return {
      paperWidth:      printerWidth,
      shopName:        order.shopName,
      branchName:      order.branchName ?? branchName,
      logoUrl:         shopLogoUrl,
      phone:           shopPhone,
      taxId:           shopTaxId,
      address:         shopAddress,
      openingHours:    shopOpeningHours,
      workingDays:     shopWorkingDays,
      googleReviewUrl: shopGoogleReviewUrl,
      ...
      receiptToken:    order.receiptToken ?? null,
  ...
  async function connectBtDevice(device: any, silent = false): Promise<boolean> {
    try {
      ...
      // ── อัพเดต state ทันทีที่ GATT เชื่อมต่อสำเร็จ ────────────────────────
      // ไม่รอหา characteristic ก่อน — ผู้ใช้เห็น ✅ ทันที
      btDeviceRef.current  = device;
      btConnectedRef.current = true;
      setBtConnected(true);
      localStorage.setItem('pos_bt_printer_name', device.name ?? '');

      // ── register disconnect listener เพียงครั้งเดียว ─────────────────────
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (!(device as any).__nexaBound) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (device as any).__nexaBound = true;
      ...
      // ── ค้นหา characteristic สำหรับปริ๊น ───────────────────────────────
      let char = null;

      // 1) ลองจาก known BT_PROFILES ก่อน
      for (const p of BT_PROFILES) {
        try { const svc = await server.getPrimaryService(p.s); char = await svc.getCharacteristic(p.c); break; }
        catch { continue; }
      }

      // 2) Fallback: scan ทุก service หา characteristic ที่เขียนได้
      if (!char) {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          outer: for (const svc of svcs) {
            ...
            for (const c of chars) {
              if (c.properties.write || c.properties.writeWithoutResponse) { char = c; break outer; }
            }
          }
        } catch { /* security restriction on insecure origin — ignore */ }
      }

      btCharRef.current = char; // null = ยังพิมพ์ไม่ได้ แต่ GATT connected แล้ว
      if (!char && !silent) toast.error('เชื่อมต่อ GATT สำเร็จ แต่ไม่พบ service ปริ๊น — ลองกดปริ๊นได้เลย หรือตรวจสอบรุ่นเครื่องปริ๊น');
      return true; // GATT connected = success
    } catch {
      btConnectedRef.current = false;
      setBtConnected(false);
      return false;
  ...
  async function printBluetooth(data: Uint8Array): Promise<boolean> {
    ...
    try {
      ...
      return true;
    } catch { btConnectedRef.current = false; setBtConnected(false); btCharRef.current = null; return false; }
  }
  ...
  async function triggerPrint(order: {
    ...
    // ── Browser print (mode = 'browser' หรือกดปุ่ม force) ───────────────────
    // เปิด print dialog ผ่าน iframe — ใช้เฉพาะ printerMode='browser' เท่านั้น
    if (printerMode !== 'browser' && !force) return;
    ...
    // Sarabun (Google Fonts) — Thai-safe, same font used by ESC/POS canvas renderer
    // browser mode ใช้เป็น fallback สำหรับ AirPrint (iOS) และ Windows print dialog
    const fontLink = `<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;700&display=swap">`;
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8">${fontLink}<style>@page{margin:0;size:${pageWidth} auto;}*{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Sarabun','Noto Sans Thai',sans-serif;font-size:13px;width:${pageWidth};padding:6px 4px;}.c{text-align:center;}.bold{font-weight:700;}.big{font-size:16px;font-weight:700;}.line{border-top:1px dashed #000;margin:4px 0;}.row{display:flex;justify-content:space-between;}.note{font-size:11px;padding-left:4px;color:#555;}</style></head><body><div class="c bold">${order.shopName}</div><div class="c">#${String(order.orderNumber).padStart(4,'0')}</div><div class="c">${new Date().toLocaleString('th-TH',{timeZone:'Asia/Bangkok',dateStyle:'short',timeStyle:'short'})}</div><div class="line"></div>${rows}<div class="line"></div>${order.discount>0?`<div class="row"><span>ส่วนลด</span><span>-${order.discount.toFixed(2)}</span></div>`:''}${order.vatEnabled?`<div class="row"><span>ก่อน VAT</span><span>${subtotal.toFixed(2)}</span></div><div class="row"><span>VAT 7%</span><span>${vatAmt.toFixed(2)}</span></div>`:''}<div class="row big"><span>รวม</span><span>${order.total.toFixed(2)} ฿</span></div><div class="line"></div><div class="c">ขอบคุณที่ใช้บริการ</div></body></html>`;
    ...
    iframe.onload = () => {
      ...
      // รอ Sarabun โหลดก่อน print — ป้องกัน Thai เป็น boxes
    ...
    <div className={isDiningUX ? 'dining-app' : 'pos-wrap'}>
      ...
      <header className="pos-topnav">
        ...
        <div className="pos-topnav__right">
          {printerEnabled && printerMode !== 'browser' && (
            printerMode === 'bluetooth' ? (
              /* BT: แสดง status chip เสมอ — auto-connect ผ่าน getDevices() ใน useEffect
                 ถ้าไม่เคยจับคู่ หรือ btConnected=false → แตะเพื่อ requestDevice() (ต้องใช้ HTTPS) */
              <button
                className={`pos-bt-btn${btConnected ? ' pos-bt-btn--on' : ''}`}
                onClick={hasBluetooth && !btConnected ? connectBluetooth : undefined}
                disabled={btConnecting || btConnected || !hasBluetooth}
                title={
                  !hasBluetooth ? 'Bluetooth ต้องใช้ HTTPS — เปิดด้วย https:// บน Chrome/Edge' :
                  btConnected   ? `✅ Bluetooth เชื่อมต่อแล้ว${btDeviceRef.current?.name ? ` — ${btDeviceRef.current.name}` : ''}` :
                  btConnecting  ? 'กำลังเชื่อมต่อ Bluetooth…' :
                                  'แตะเพื่อจับคู่เครื่องปริ๊น Bluetooth'
                }
              >
                {!hasBluetooth ? '⚠️' : btConnecting ? '⏳' : btConnected ? '✅' : '📶'}
                <span className="pos-bt-btn__dot" />
              </button>
            ) : (
              /* USB / Network: ปุ่ม connect ปกติ */
              <button
              ...
              >
                {printerConnecting ? '⏳' : printerConnected ? '✅' : printerMode === 'network' ? '🌐' : '🔌'}
                <span className="pos-bt-btn__dot" />
              </button>
            )
          )}
