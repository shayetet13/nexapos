...
export function CustomersPanel({ shopId, token, selectedCustomer, onSelect, cartTotal, onClose }: Props) {
  ...
  function handleRegisterClick() {
    setShowRegisterQr(prev => !prev);
    void openRegisterOnDisplay();
  }
  ...
  return (
    <div className="cust-panel">
      ...
      >
        {showRegisterQr ? '✕ ซ่อน QR สมัครสมาชิก' : '📲 ให้ลูกค้าสมัครสมาชิก'}
      </button>

      {showRegisterQr && registerUrl && (
        <div className="cust-register-qr">
          <p className="cust-register-qr__hint">ให้ลูกค้าสแกน QR เพื่อสมัครสมาชิก</p>
          <div className="cust-register-qr__code">
            <QRCode value={registerUrl} size={160} bgColor="#ffffff" fgColor="#6d28d9" level="M" />
          </div>
          <p className="cust-register-qr__url">{registerUrl}</p>
        </div>
      )}
