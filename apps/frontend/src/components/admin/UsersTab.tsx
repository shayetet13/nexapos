...
/** PIN validation: digits only, exactly 4 chars */
...
export function UsersTab({
  ...
  const invitePassVis = usePinVisible();
        ...
        <div className="page-admin__form">
          ...
          <div>
            ...
            <div className="page-admin__pin-input-wrap">
              <input
                type={invitePassVis.visible ? 'text' : 'password'}
                placeholder="เช่น Abc123! (4–50 ตัว)"
                value={invitePassword}
                onChange={(e) => setInvitePassword(e.target.value.slice(0, 50))}
                className="input-field"
                autoComplete="new-password"
              />
              <button type="button" onClick={invitePassVis.toggle} className="page-admin__pin-toggle" title="แสดง/ซ่อนรหัสผ่าน">
                {invitePassVis.visible ? '🙈' : '👁️'}
              </button>
            </div>
            <p className="page-admin__password-hint">
              ยาว 4–50 ตัวอักษร · ใช้ได้ทั้งตัวพิมพ์เล็ก ตัวพิมพ์ใหญ่ ตัวเลข และอักขระพิเศษ
          ...
          <div className="page-admin__form-actions">
            <button
              type="button"
              onClick={inviteUser}
              disabled={inviting || !inviteEmail.trim() || invitePassword.length < 4}
      ...
      <div className="page-admin__card">
        ...
        <div className="page-admin__form">
          <div className="page-admin__invite-row">
            ...
            <div className="page-admin__invite-field">
              <label className="page-admin__label">PIN (4 หลักเท่านั้น)</label>
              <div className="page-admin__pin-input-wrap">
                <input
              ...
              {staffPin.length > 0 && !validPin(staffPin) && (
                <p className="page-admin__error-hint">PIN ต้องเป็นตัวเลข 4 หลักเท่านั้น ({staffPin.length}/4 หลัก)</p>
              )}
            </div>
          </div>
          <div className="page-admin__invite-row">
          ...
          <div className="page-admin__modal">
            ...
            <div className="page-admin__form">
              {editStaffTab === 'nickname' ? (
                ...
                <div>
                  <label className="page-admin__label">PIN ใหม่ (4 หลักเท่านั้น)</label>
                  <div className="page-admin__pin-input-wrap">
                    <input
                      type={editPinVis.visible ? 'text' : 'password'}
                      placeholder="ตัวเลข 4 หลัก"
                      value={editStaffPin}
                      onChange={(e) => setEditStaffPin(e.target.value.replace(/\D/g, '').slice(0, 4))}
                      className="input-field"
                      inputMode="numeric"
                      autoComplete="new-password"
                      maxLength={4}
                  ...
                  {editStaffPin.length > 0 && !validPin(editStaffPin) && (
                    <p className="page-admin__error-hint">PIN ต้องเป็นตัวเลข 4 หลักเท่านั้น ({editStaffPin.length}/4 หลัก)</p>
                  )}
                </div>
              )}
