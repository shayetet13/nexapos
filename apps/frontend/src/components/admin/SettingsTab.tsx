...
import { loadReceiptLayout, saveReceiptLayout } from '@/lib/receipt-layout';
...
export function SettingsTab({
  ...
  // ── Receipt logo toggle (device receipt layout) ───────────────
  const [printLogo, setPrintLogo] = useState<boolean>(() => loadReceiptLayout().showLogo);
  function togglePrintLogo(next: boolean) {
    setPrintLogo(next);
    saveReceiptLayout({ ...loadReceiptLayout(), showLogo: next });
  }

  // ── Branch management local state ─────────────────────────────
  const [branchAddOpen, setBranchAddOpen] = useState(false);
  const [branchAddName, setBranchAddName] = useState('');
  const [branchAddAddress, setBranchAddAddress] = useState('');
  const [branchAddSaving, setBranchAddSaving] = useState(false);
  ...
  async function addBranch() {
    if (!shopId || !branchAddName.trim()) return;
    setBranchAddSaving(true);
    try {
      const res = await fetchWithAuth(`${API_URL}/api/v1/shops/${shopId}/branches`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: branchAddName.trim(), address: branchAddAddress.trim() || null }),
      });
      if (res.ok) {
        const j = await res.json() as { data: Branch };
        onBranchesChange([...branches, j.data]);
        setBranchAddName('');
        setBranchAddAddress('');
        setBranchAddOpen(false);
        toast.success(`เพิ่มสาขา "${j.data.name}" เรียบร้อยแล้ว`);
      } else {
        const j = await res.json().catch(() => ({})) as { error?: { message?: string } };
        toast.error(j.error?.message ?? 'เพิ่มสาขาไม่สำเร็จ');
      }
    } finally {
      setBranchAddSaving(false);
    }
  }

  function openEditBranch(b: Branch) {
    setBranchEditId(b.id);
    setBranchEditName(b.name);
    setBranchEditAddress(b.address ?? '');
  }

  async function saveEditBranch() {
    if (!shopId || !branchEditId || !branchEditName.trim()) return;
    setBranchEditSaving(true);
    try {
      const res = await fetchWithAuth(`${API_URL}/api/v1/shops/${shopId}/branches/${branchEditId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: branchEditName.trim(), address: branchEditAddress.trim() || null }),
      });
      if (res.ok) {
        const j = await res.json() as { data: Branch };
        onBranchesChange(branches.map((b) => (b.id === branchEditId ? j.data : b)));
        setBranchEditId(null);
        toast.success('แก้ไขสาขาเรียบร้อยแล้ว');
      } else {
        const j = await res.json().catch(() => ({})) as { error?: { message?: string } };
        toast.error(j.error?.message ?? 'แก้ไขสาขาไม่สำเร็จ');
      }
    } finally {
  ...
  async function deleteBranch(b: Branch) {
    ...
    const ok = await confirm({
      title: 'ลบสาขา',
      description: <><strong>{b.name}</strong> จะถูกลบออกจากระบบถาวร<br />ข้อมูลออเดอร์และสต๊อกที่ผ่านมาจะยังคงอยู่</>,
      variant: 'danger',
      icon: '🏪',
      confirmLabel: 'ลบสาขา',
    });
    if (!ok) return;
    const res = await fetchWithAuth(`${API_URL}/api/v1/shops/${shopId}/branches/${b.id}`, { method: 'DELETE' });
    if (res.ok) {
      onBranchesChange(branches.filter((br) => br.id !== b.id));
      toast.success(`ลบสาขา "${b.name}" เรียบร้อยแล้ว`);
    } else {
      const j = await res.json().catch(() => ({})) as { error?: { message?: string } };
      toast.error(j.error?.message ?? 'ลบสาขาไม่สำเร็จ');
    }
  }
    ...
    <div className="page-admin__tab-content">
      ...
      <div className="page-admin__card">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
          <h3 className="page-admin__card-title" style={{ marginBottom: 0 }}>🏪 จัดการสาขา</h3>
          <button
            type="button"
            className="btn-primary"
            style={{ fontSize: '0.8rem', padding: '0.35rem 0.9rem' }}
            onClick={() => { setBranchAddOpen((v) => !v); setBranchAddName(''); setBranchAddAddress(''); }}
          >
            {branchAddOpen ? '✕ ยกเลิก' : '+ เพิ่มสาขา'}
          </button>
        </div>
        <p className="page-admin__section-desc">ดูและจัดการสาขาร้าน — ชื่อสาขา ที่อยู่ และสถานะ</p>

        {/* Add branch inline form */}
        {branchAddOpen && (
          <div className="page-admin__branch-form">
            <div className="page-admin__form" style={{ gap: '0.6rem' }}>
              <div>
                <label className="page-admin__label">ชื่อสาขา <span style={{ color: '#ef4444' }}>*</span></label>
                <input
                  type="text"
                  placeholder="เช่น สาขาสยาม, สาขาเซ็นทรัล"
                  value={branchAddName}
                  onChange={(e) => setBranchAddName(e.target.value)}
                  className="input-field"
                  maxLength={100}
                />
              </div>
              <div>
                <label className="page-admin__label">ที่อยู่สาขา (ไม่บังคับ)</label>
                <input
                  type="text"
                  placeholder="เช่น ชั้น 3 ห้าง Siam Paragon กรุงเทพฯ"
                  value={branchAddAddress}
                  onChange={(e) => setBranchAddAddress(e.target.value)}
                  className="input-field"
                  maxLength={500}
                />
              </div>
              <div>
                <button
                  type="button"
                  className="btn-primary"
                  disabled={branchAddSaving || !branchAddName.trim()}
                  onClick={() => void addBranch()}
                >
                  {branchAddSaving ? 'กำลังเพิ่ม...' : 'เพิ่มสาขา'}
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Branch list */}
        {branches.length === 0 ? (
          <p style={{ color: '#9ca3af', fontSize: '0.85rem', marginTop: '0.75rem' }}>ยังไม่มีสาขา กดปุ่มเพิ่มสาขาเพื่อเริ่มต้น</p>
        ) : (
          <div className="page-admin__branch-list">
            {branches.map((b) => (
              <div key={b.id} className="page-admin__branch-row">
                {branchEditId === b.id ? (
                  /* Edit inline form */
                  <div style={{ flex: 1 }}>
                    <div className="page-admin__form" style={{ gap: '0.5rem' }}>
                      <div>
                        <label className="page-admin__label">ชื่อสาขา <span style={{ color: '#ef4444' }}>*</span></label>
                        <input
                          type="text"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setBranchEditAddress(e.target.value)}
                          className="input-field"
                          maxLength={500}
                        />
                      </div>
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button
                          ...
                          disabled={branchEditSaving || !branchEditName.trim()}
                  ...
                  /* Display row */
                  <>
                    <div className="page-admin__branch-info">
                      <div className="page-admin__branch-name">
                        {b.name}
                        <span className={`page-admin__branch-badge${b.is_active ? '' : ' page-admin__branch-badge--inactive'}`}>
                          {b.is_active ? 'เปิด' : 'ปิด'}
                        </span>
                      </div>
                      {b.address && (
                        <div className="page-admin__branch-address">📍 {b.address}</div>
                      )}
                    </div>
                    <div className="page-admin__branch-actions">
                      <button
                        type="button"
                        className="page-admin__btn-sm"
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (!shopId) return;
                          const ok = await confirm({
                            title: 'ลบส่วนลดสำเร็จรูป',
                            description: <><strong>{p.name}</strong> จะถูกลบออกจากระบบถาวร</>,
                            variant: 'danger',
                            icon: '🗑',
                            confirmLabel: 'ลบ',
                          });
                          ...
                          const res = await fetchWithAuth(
                            `${API_URL}/api/v1/shops/${shopId}/promotions/${p.id}`,
                            { method: 'DELETE' },
                          );
                          if (res.ok) {
                            setPromoPresets((prev) => prev.filter((x) => x.id !== p.id));
                            if (promoFormId === p.id) {
                              setPromoFormId(null);
                              setPromoFormName('');
                              setPromoFormValue(0);
                            ...
                            toast.success(`ลบ "${p.name}" เรียบร้อยแล้ว`);
                          } else {
                            const j = await res.json().catch(() => null) as { error?: { message?: string } } | null;
                            toast.error(j?.error?.message ?? 'ลบส่วนลดไม่สำเร็จ');
                          }
                        }}
                      >
                        🗑 ลบ
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const ok = await confirm({
                            title: 'ลบชุดเซ็ต',
                            description: <><strong>{c.name}</strong> จะถูกลบออกจากระบบถาวร</>,
                            variant: 'danger',
                            icon: '🗑',
                          ...
                          try {
                            const res = await fetchWithAuth(
                              `${API_URL}/api/v1/shops/${shopId}/combos/${c.id}`,
                              { method: 'DELETE' },
                            );
                            if (res.ok) {
                              setPromoCombos((prev) => prev.filter((x) => x.id !== c.id));
                              if (comboFormId === c.id) {
                                setComboFormId(null);
                                setComboFormName('');
                                setComboFormPrice(0);
                              ...
                              toast.success(`ลบ "${c.name}" เรียบร้อยแล้ว`);
                            } else {
                              const j = await res.json().catch(() => null) as { error?: { message?: string } } | null;
      ...
      {settingsPrintEnabled && (
        <div className="page-admin__printer-section">
          ...
          {/* พิมพ์โลโก้ร้านบนใบเสร็จ */}
          <div className="page-admin__vat-row">
            <div className="page-admin__vat-info">
              <span className="page-admin__label">พิมพ์โลโก้บนใบเสร็จ</span>
              <span className="page-admin__vat-desc">
                {printLogo ? 'เปิดอยู่ — ใบเสร็จจะมีโลโก้ร้านด้านบน' : 'ปิดอยู่ — ไม่พิมพ์โลโก้ (ประหยัดหมึก/กระดาษ)'}
              </span>
            </div>
            <button
              type="button"
              role="switch"
              aria-checked={printLogo}
              onClick={() => togglePrintLogo(!printLogo)}
              className={`page-admin__toggle${printLogo ? ' page-admin__toggle--on' : ''}`}
            >
              <span className="page-admin__toggle-thumb" />
            </button>
          </div>
          ...
          <div className="page-admin__printer-card">
            ...
            {/* Thai code page hardcoded CP874 (ESC t 20) — not configurable */}
