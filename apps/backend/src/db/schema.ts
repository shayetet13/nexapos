...
export const shops = pgTable('shops', {
  ...
  /** retail = POS ขายตรง; full_service_restaurant = โต๊ะ + เปิดบิล */
  shop_mode: text('shop_mode', { enum: ['retail', 'full_service_restaurant'] })
    .notNull()
    .default('retail'),
...
export const diningTables = pgTable(
  ...
  (t) => [
    index('dining_tables_shop_id_idx').on(t.shop_id),
    uniqueIndex('dining_tables_shop_branch_label_idx').on(t.shop_id, t.branch_id, t.label),
  ],
);
...
export const diningSessions = pgTable(
  'dining_sessions',
  {
    ...
    shop_id:         uuid('shop_id').notNull().references(() => shops.id, { onDelete: 'cascade' }),
    branch_id:       uuid('branch_id').notNull().references(() => branches.id, { onDelete: 'cascade' }),
    dining_table_id: uuid('dining_table_id').notNull().references(() => diningTables.id, { onDelete: 'cascade' }),
    status:          text('status', { enum: ['open', 'closed', 'void'] }).notNull().default('open'),
  ...
  (t) => [
    index('dining_sessions_shop_status_idx').on(t.shop_id, t.status),
    index('dining_sessions_table_status_idx').on(t.dining_table_id, t.status),
  ],
);
...
export const orders = pgTable(
  'orders',
  {
    ...
    /** ออเดอร์รอบสั่งในโหมดภัตตาคาร (pending จนกว่าจะปิดเซสชัน) */
    dining_session_id: uuid('dining_session_id').references(() => diningSessions.id, { onDelete: 'set null' }),
  ...
  (t) => [
    ...
    index('orders_dining_session_id_idx').on(t.dining_session_id),
  ]
);
