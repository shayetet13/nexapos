...
/** Deduplicate key stored in notification title prefix — one per shop per alert bucket per ISO week */
function dedupeKey(shopId: string, bucket: number): string {
  const now = bkkNow();
  const week = String(isoWeek(now)).padStart(2, '0');
  // e.g. "__sub_<shopId>_7_W2026-17" — fires at most once per shop per bucket per week
  return `__sub_${shopId}_${bucket}_W${now.getFullYear()}-${week}`;
}
...
export async function runSubscriptionExpiryNotifications() {
  ...
  for (const row of rows) {
    ...
    for (const bucket of ALERT_DAYS) {
      ...
      // Check if this week's notification for this shop+bucket already exists (last 7 days)
      ...
      const existing = await db
        ...
        .where(
          and(
            ...
            gte(shopNotifications.created_at, weekAgo),
