...
await app.register(cors, {
  origin: (origin: string | undefined, cb: (err: Error | null, allow: boolean) => void) => {
    // No origin = curl / mobile app / server-side → allow
    if (!origin) return cb(null, true);
    // Explicit allowlist (production + dev static)
    if (allowedOrigins.includes(origin)) return cb(null, true);
    // Dev mode: allow any LAN IP, localhost variant, cloudflare/ngrok tunnel
    // (consistent with internal-token LAN bypass — no token required on private IPs in dev)
    if (isDev) {
      const isLan     = /^https?:\/\/(localhost|127\.0\.0\.1|192\.168\.\d+\.\d+|10\.\d+\.\d+\.\d+)(:\d+)?$/.test(origin);
      const isTunnel  = /\.trycloudflare\.com$/.test(origin) || /\.ngrok(-free)?\.app$/.test(origin) || /\.ngrok\.io$/.test(origin);
      if (isLan || isTunnel) return cb(null, true);
    }
