import { eq, and, gte, lte, sql, desc, asc, SQL, ilike, type AnyColumn } from 'drizzle-orm';
import { randomUUID } from 'crypto';
import { db } from '../db/index.js';
import { orders, orderItems, products, branches, users, shops } from '../db/schema.js';

/**
 * `timestamptz` อยู่ในวันธุรกิจเดียวกับ "ตอนนี้" (ม้วนวันที่ 00:15 น. Asia/Bangkok).
 * ใช้ template เดียว ไม่ nested `sql\`\${a} = \${b}\`` ลดความเสี่ยงบั๊ก driver.
 */
function sqlPosBkkSameBusinessDayAsNow(col: AnyColumn) {
  return sql`(
    (CASE
      WHEN (${col} AT TIME ZONE 'Asia/Bangkok')::time < TIME '00:15:00'
      THEN (${col} AT TIME ZONE 'Asia/Bangkok')::date - 1
      ELSE (${col} AT TIME ZONE 'Asia/Bangkok')::date
    END) = (
    CASE
      WHEN (NOW() AT TIME ZONE 'Asia/Bangkok')::time < TIME '00:15:00'
...
export const orderRepository = {
  ...
  async create(data: {
    ...
    // Daily sequence per shop — rolls at 00:15 Asia/Bangkok (same as POS)
    const [dailyRow] = await db
      ...
      .where(
        and(
          ...
          sqlPosBkkSameBusinessDayAsNow(orders.created_at),
        )
      );
  ...
  listPendingByDiningSession(shopId: string, sessionId: string) {
    return db
      .select()
      .from(orders)
      .where(
        and(
          ...
          eq(orders.dining_session_id, sessionId),
  ...
  /** ออเดอร์ของสาขานี้ "วันนี้" (POS) = business date, rolls 00:15 Asia/Bangkok */
  ...
  /** นับออเดอร์ "วันนี้" (POS) ของร้าน — ม้วน 00:15 Asia/Bangkok */
  ...
  async getPnlReport(
    ...
    // Convert to Asia/Bangkok before truncating so each calendar day in Bangkok
    // is grouped correctly (without AT TIME ZONE, orders between Bangkok 00:00–06:59
    // would be bucketed into the previous UTC day).
    const truncLit = sql.raw(`'${groupBy === 'month' ? 'month' : 'day'}'`);
    const periodExpr = sql<string>`DATE_TRUNC(${truncLit}, ${orders.created_at} AT TIME ZONE 'Asia/Bangkok')::date`;
    ...
    // ── Revenue per period — query orders only (no join) ─────────
    // Joining orderItems before summing orders.total causes JOIN multiplication:
    // each order appears N times (once per item row), inflating the revenue total.
    ...
    // ── COGS per period — query orderItems (correct: one row per item) ─
    ...
    // Merge by period key
    const cogsMap = new Map(cogsRows.map((r) => [String(r.period), Number(r.cogs)]));
    const rows = revenueRows.map((r) => ({
      period:      String(r.period),
      order_count: r.order_count,
      revenue:     r.revenue,
      cogs:        String(cogsMap.get(String(r.period)) ?? 0),
    }));
  ...
  async getStats(shopId: string, opts?: { fromDate?: Date; toDate?: Date; branchId?: string }) {
    // All "start of period" dates use Asia/Bangkok UTC+7, with the same 00:15 business-day
    // rollover as sqlPosBkkSameBusinessDayAsNow, so comparison cards are consistent with
    // the "วันนี้" card and the period hero card.
    const BKK_OFFSET_MS = 7 * 3600_000;
    const BIZ_DELAY_MS  = 15 * 60_000;  // 00:15 rollover

    const nowMs  = Date.now();
    const nowBkk = new Date(nowMs + BKK_OFFSET_MS); // Bangkok wall-clock as UTC fields
    const isBefore0015 = nowBkk.getUTCHours() === 0 && nowBkk.getUTCMinutes() < 15;
    const bizBkk = isBefore0015 ? new Date(nowMs + BKK_OFFSET_MS - 86_400_000) : nowBkk;

    const by = bizBkk.getUTCFullYear();
    const bm = bizBkk.getUTCMonth();
    const bd = bizBkk.getUTCDate();

    // Returns the UTC instant corresponding to 00:15 Asia/Bangkok on the given Bangkok calendar date
    const bkkBizStart = (y: number, mo: number, day: number) =>
      new Date(Date.UTC(y, mo, day) - BKK_OFFSET_MS + BIZ_DELAY_MS);

    const startOfToday = bkkBizStart(by, bm, bd);
    const startOfWeek  = bkkBizStart(by, bm, bd - bizBkk.getUTCDay());
    const startOfMonth = bkkBizStart(by, bm, 1);
    const startOfYear  = bkkBizStart(by, 0, 1);

    const { fromDate, toDate, branchId } = opts ?? {};
    const branchCond = branchId ? [eq(orders.branch_id, branchId)] : [];

    // ── Summary (optional branch) — ยอด "วันนี้" รายวันตรงกับ POS (รีเซ็ต 00:15 Asia/Bangkok)
    ...
    const periodTo   = toDate   ?? new Date();
    const periodBase = [
      eq(orders.shop_id, shopId),
      eq(orders.status, 'paid'),
    ...
    const fetchMoneyMistake = async (from: Date, to?: Date, usePosBusinessDay = false) => {
      const conditions = [
        eq(orders.shop_id, shopId),
      ...
      if (usePosBusinessDay) {
        conditions.push(sqlPosBkkSameBusinessDayAsNow(orders.refunded_at));
      } else {
        conditions.push(gte(orders.refunded_at, from));
    ...
    const [mmDaily, mmMonthly, mmYearly] = await Promise.all([
      fetchMoneyMistake(startOfToday, undefined, true),
