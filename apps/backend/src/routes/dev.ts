...
import { eq, sql, desc, gte, lte, and, count, isNotNull } from 'drizzle-orm';
...
// DELETE /dev/branches/:branchId
  ...
  // POST /dev/shops/:shopId/products/restore — restore all soft-deleted products for a shop
  app.post<{ Params: { shopId: string } }>(
    '/shops/:shopId/products/restore',
    { preHandler: [app.auth] },
    async (req, reply) => {
      await requireDevAdmin(req);
      const restored = await db
        ...
        .where(and(eq(products.shop_id, req.params.shopId), isNotNull(products.deleted_at)))
        .returning();
      return reply.send({ success: true, data: { restored: restored.length }, meta: meta(req) });
    },
  );
