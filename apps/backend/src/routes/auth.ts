...
const registerSchema = z.object({
  ...
  shop_mode:       z.enum(['retail', 'full_service_restaurant']).default('retail'),
...
const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Body: unknown }>('/register', {
    ...
    const { email, password, verified_token, shopName, shop_mode, province, district, postal_code, branchName } = parsed.data;
    ...
    let shop: Awaited<ReturnType<typeof shopRepository.createShop>>;
    try {
      // 3. Create shop
      shop = await shopRepository.createShop(shopName, {
      ...
      const pgc  = (err as { code?: string }).code;
      req.log.error({ err }, 'register: shop/branch/assign failed');
      // PG undefined_column 42703: มักเพราะยังไม่ migrate (เช่น ไม่มี shop_mode)
      if (pgc === '42703' || /shop_mode|dining_|\bcolumn\b.*does not exist/i.test(msg)) {
        try {
        ...
        return reply.status(503).send({
          ...
          error: {
            code:    'DB_SCHEMA',
            message: 'ฐานข้อมูลยังไม่อัปเดต schema ล่าสุด กรุณารัน `pnpm db:migrate` หรือ `pnpm db:push` ที่เครื่องเซิร์ฟเวอร์/CI แล้วลองสมัครใหม่',
          },
          meta: meta(req),
        });
      }
  ...
  app.post<{ Body: unknown }>('/staff-login', {
    config: { rateLimit: { max: 10, timeWindow: '1 minute' } },
  }, async (req, reply) => {
    ...
    if (!staffUser) {
      req.log.warn('staff-login: no staff row for nickname (lookup miss)');
    ...
    // รูปแบบใหม่: prefix + PIN (ผ่านนโยบายรหัสเมื่อเปลี่ยน PIN) · legacy: PIN ตรงจาก createUser เดิม
    ...
    if (session.error || !session.data.session) {
      req.log.warn(
        { supabaseSignIn: session.error?.message ?? 'no_session' },
        'staff-login: Supabase signIn failed after derived + legacy password',
      );
      return reply.status(401).send({
        success: false,
        error: { message: 'ชื่อเล่นหรือ PIN ไม่ถูกต้อง' },
