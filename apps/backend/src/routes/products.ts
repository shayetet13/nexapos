...
import { requireAdminShop, requireFeature, requirePlanLimit } from '../lib/admin-guard.js';
...
const productsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Params: { shopId: string }; Body: unknown }>('/shops/:shopId/products', {
    ...
    const countRows = await db
      .select({ value: count() })
      .from(productsTable)
      .where(eq(productsTable.shop_id, req.params.shopId));
  ...
  app.delete<{ Params: { shopId: string; productId: string } }>(
    '/shops/:shopId/products/:productId', {
      ...
      try {
        await productRepository.delete(req.params.productId, req.params.shopId);
      } catch (err: unknown) {
        // PostgreSQL foreign key violation: product is referenced by order_items (onDelete: restrict)
        const pgErr = err as { code?: string };
        if (pgErr?.code === '23503') {
          return reply.status(409).send({
            error: {
              message: 'ไม่สามารถลบสินค้านี้ได้ เนื่องจากมีออเดอร์ที่อ้างอิงถึงสินค้านี้อยู่',
            },
          });
        }
        throw err;
  ...
  app.put<{
    ...
    const [branch, product] = await Promise.all([
      ...
      productRepository.getById(productId, shopId),
    ]);
  ...
  app.post<{ Params: { shopId: string }; Body: unknown }>(
    '/shops/:shopId/stock/transfer', {
      ...
      const [fromBranch, toBranch, product] = await Promise.all([
        ...
        productRepository.getById(product_id, shopId),
      ]);
