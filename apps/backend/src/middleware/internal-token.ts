...
/** เบราว์เซอร์ dev ชี้ Fastify ที่ localhost โดยตรง — ไม่มี CF Worker ใส่ X-Internal-Token */
function skipInternalTokenForLocalDirectBrowser(request: FastifyRequest): boolean {
  ...
  const fromLocalHost =
    ...
    host.startsWith('[::1]');
  if (!fromLocalHost) return false;
  // tsx dev มักไม่ตั้ง NODE_ENV → ถือว่าไม่ใช่ production ชัดเจน
