...
export type CreateOrderResult =
  | {
      ...
      items: Array<{ product_id: string; name: string; quantity: number; unit_price: number; subtotal: number; note?: string }>;
    }
...
export async function createOrder(
  ...
  if (isTab) {
    if (shop?.shop_mode !== 'full_service_restaurant') {
      return { success: false, error: { code: 'SHOP_MODE', message: 'ร้านนี้ไม่ได้ตั้งค่าโหมดภัตตาคาร' } };
    }
    const session = await diningSessionRepository.getById(diningSessionId, shopId);
    if (!session || session.status !== 'open') {
      return { success: false, error: { code: 'SESSION', message: 'ไม่พบเซสชันโต๊ะ หรือปิดบิลแล้ว' } };
    }
    if (session.branch_id !== branchId) {
      return { success: false, error: { code: 'VALIDATION_ERROR', message: 'สาขาไม่ตรงกับเซสชัน' } };
    }
  }

  // Fetch customer + products + stock in parallel (single round-trip)
  const productIds = items.map((i: { product_id: string; quantity: number; note?: string }) => i.product_id);
  const [customer, allProducts, allStock] = await Promise.all([
    !isTab && customer_id ? customerRepository.getById(shopId, customer_id) : Promise.resolve(null),
    productRepository.getByIds(shopId, productIds),
    stockRepository.getStockForProducts(branchId, productIds),
  ]);

  // Validate points_redeemed against customer balance (full checkout only)
  let validatedPointsRedeemed = 0;
  if (!isTab && customer_id && points_redeemed > 0 && customer && customer.points >= points_redeemed) {
    validatedPointsRedeemed = points_redeemed;
  }
  let discount = isTab ? 0 : pointsToDiscountFromConfig(validatedPointsRedeemed, membershipConfig);
  ...
  for (const item of items) {
    ...
    /* รอบภัตตาคาร (open tab): ไม่จำกัดจำนวนตามสต็อกหน้าร้าน — ครัวทำตามออร์เดอร์ */
    if (!isTab && stock !== null && stock.quantity < item.quantity) {
      return {
        success: false,
        error: {
  ...
  if (!isTab && customer_id && customer && (membershipConfig?.birthday_benefit_type === 'percent' || membershipConfig?.birthday_benefit_type === 'fixed') && typeof membershipConfig?.birthday_benefit_value === 'number' && membershipConfig.birthday_benefit_value > 0) {
  ...
  const pointsEarned = !isTab && customer_id ? calcPointsEarned(finalTotal, pointsPer10) : 0;

  const order = await orderRepository.create({
    shop_id:             shopId,
    branch_id:         branchId,
    user_id:             userId,
    customer_id:         isTab ? undefined : (customer_id ?? undefined),
    total:               finalTotal.toFixed(2),
    discount:            discount.toFixed(2),
    points_earned:       pointsEarned,
    points_redeemed:     isTab ? 0 : validatedPointsRedeemed,
    payment_method:      isTab ? undefined : payment_method,
    cash_received:       isTab ? undefined : ((payment_method === 'cash' && cash_received != null) ? cash_received.toFixed(2) : undefined),
    status:              isTab ? 'pending' : 'paid',
    dining_session_id:   isTab ? diningSessionId : null,
  });
  ...
  // Update customer points and tier after order (not on open-tab rounds)
  if (!isTab && customer_id && order) {
  ...
  if (isTab) {
    broadcast(shopId, 'ORDER_TAB_ROUND', {
      order_id:  order.id,
      branch_id: branchId,
  ...
  broadcast(shopId, 'ORDER_CREATED', {
    ...
    total:     finalTotal,
    items:     orderItems,
    ...(isTab ? { pending: true as const, dining_session_id: diningSessionId } : {}),
  });
  ...
  return {
    ...
    paymentMethod: isTab ? 'open_tab' : (payment_method ?? 'other'),
    cashReceived:  isTab ? null : ((payment_method === 'cash' && cash_received != null) ? Number(cash_received) : null),
    total:         finalTotal,
    discount,
    orderStatus:   isTab ? 'pending' : 'paid',
